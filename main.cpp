...
//	const auto DICT_FILENAME = "dict.csv";
	...
	{
		auto cleanDefSentences = makeCleanSentences(w.definitions);
		auto cleanDefWords = makeCleanWords(cleanDefSentences);
	...
	auto getRandModel = [&rne, &wordToCleanDefWords, &getRandDictWord](void) {
		...
		return std::make_pair(randWord, makeMarkovModel(wordToCleanDefWords[randWord.name], N_GRAM));
	};

	auto getRandState = [&rne](const MarkovModel &model) {
	...
	MarkovModel curModel;
	Word startingWord;
	while(true)
	{
		startingWord = getRandDictWord();		
		std::println("Trying {}...", startingWord.name);
		curModel = makeMarkovModel(wordToCleanDefWords[startingWord.name], N_GRAM);
